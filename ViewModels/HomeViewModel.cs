    ...
    public class HomeViewModel : AsyncLoadViewModel<IList<MainInit>> {
        ...
        public override async Task<RespBase<IList<MainInit>>> OnLoadAsync() {
            try {
                ...
                var mainInitConfigFile = await tempFolder.CreateFileAsync("mainInitConfigFile.txt", CreationCollisionOption.ReplaceExisting);
                ...
                var tempFolder = ApplicationData.Current.TemporaryFolder;
                try {
                    var mainInitConfigFile = await tempFolder.GetFileAsync("mainInitConfigFile.txt");
